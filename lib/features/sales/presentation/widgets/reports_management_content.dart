...
class ReportsManagementContent extends ConsumerStatefulWidget {
  const ReportsManagementContent({super.key});

  @override
  ConsumerState<ReportsManagementContent> createState() => _ReportsManagementContentState();
}

class _ReportsManagementContentState extends ConsumerState<ReportsManagementContent> with SingleTickerProviderStateMixin {
      ...
      body: Padding(
        ...
        child: Column(
          children: [
            ...
            Expanded(child: _buildTabContent(state, provider)),
          ],
        ),
      ),
    );
  ...
  Widget _buildTabHeader(ReportState state) {
    return Column(
      ...
      children: [
        ...
        if (_currentTabIndex == 0) _buildListHeader(state),
      ],
    );
  }
  ...
  Widget _buildListHeader(ReportState state) {
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          ElevatedButton.icon(
            onPressed: () => _showCreateDialog(context, ref.read(reportProvider.notifier)),
  ...
  Widget _buildTabContent(ReportState state, ReportProvider provider) {
    switch (_currentTabIndex) {
      case 0:
        return ReportListWidget(
          showFilters: true,
          showCreateButton: false,
          onCreateReport: () => _showCreateDialog(context, provider),
          onViewReport: (report) => _showDetailDialog(context, report, provider),
          onEditReport: (report) => _showEditDialog(context, report, provider),
        );
      case 1:
  ...
  void _showCreateDialog(BuildContext context, ReportProvider provider) {
    showDialog(
      context: context,
  ...
  void _showEditDialog(BuildContext context, Report report, ReportProvider provider) {
    showDialog(
      context: context,
      builder: (context) => Dialog(
        insetPadding: const EdgeInsets.all(20),
  ...
  void _showDetailDialog(BuildContext context, Report report, ReportProvider provider) {
    showDialog(
      context: context,
      builder: (context) => Dialog(
        insetPadding: const EdgeInsets.all(20),
        child: ConstrainedBox(
          ...
          child: ReportDetailWidget(
            ...
            onEdit: () {
              Navigator.pop(context);
              _showEditDialog(context, report, provider);
            },
            onDelete: () {
              Navigator.pop(context);
